...
import {FileOpenDialog} from "file-open-dialog/file-open-dialog.js";

const APP_NAME = "gsciter.app";
...
const fileFilter = [
  { caption:"Supported files", masks: ["*.htm*","*.svg","*.zip","*.scapp"] },
  { caption:"HTML files", masks: ["*.htm*"] },
  { caption:"SVG files", masks: ["*.svg"] },
  { caption:"SCAPP files", masks: ["*.zip","*.scapp"] },
  { caption:"All files", masks: ["*.*"] },
];
...
on("click","button#open", function() {
    const options = {
      id: "open-files",
      filter: fileFilter,
      caption: "Open HTML application file"
    }
    FileOpenDialog.selectFile(function(path) {
      //console.log("path", path);
      if( path ) 
        loadFile(URL.fromPath(path));
    },options);
...
on("click", "button#open-in-window", function() {
  var fn = window.selectFile("open",file_filter);
  if( fn ) 
    window.load(fn);
})
