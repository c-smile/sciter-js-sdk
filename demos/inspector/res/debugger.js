...
function Breakpoints(props) {
  ...
  const list = channel.breakpoints.map((bp) => {
    return <Breakpoint channel={channel} breakpoint={bp} />;
  });
...
export class DebuggerView extends View {
  ...
  render() {
    ...
    const atts = atbreakpoint ? {awaiting: true} : {};
  ...
  ["on click at #continue"]() {
    this.channel.atBreakpointResponse("step", 0);
  }
  ["on click at #step-in"]() {
    this.channel.atBreakpointResponse("step", 1);
  }
  ["on click at #step-out"]() {
    this.channel.atBreakpointResponse("step", 2);
  }
  ["on click at #step-over"]() {
    this.channel.atBreakpointResponse("step", 3);
  }
  ...
  renderVariables() {
    ...
    const {local, closure, global} = this.channel.variables;
    local.caption = "local";
...
*/
  ...
  ["on click at b.funcname"](evt, b) {
    const li = b.$p("li");
    const index = parseInt(li.getAttribute("key"));
