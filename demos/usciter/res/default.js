...
function updateCaption() {
    const croot = content.frame.document;
    if (!croot) return;
    let title = croot.$("head>title");
    if (title) {
        title = `uSciter: ${title.text || ""}`;
        view.windowCaption = title;
        const capEl = $("window-caption");
        if (capEl)
...
on("click", "button#inspector", async function() {
    ...
    const inspectorPath = home((PREFIX[PLATFORM] || "") + "inspector" + (SUFFIX[PLATFORM] || ""));
